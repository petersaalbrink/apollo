...
from common import dateformat, flatten, levenshtein
...
def test_dateformat(d: int,
                    m: int,
                    y: int,
                    s: str,
                    year_first: bool):
    date_string = f"{y}{s}{m:02d}{s}{d:02d}" if year_first else f"{d:02d}{s}{m:02d}{s}{y}"
    try:
