...
postbussen = ['postbus', 'po box', 'post bus', 'vo box', 'pastbus', 'past bus', 'p.o. box']
remove_list = ['no.', 'no', 'restaurant', '7-eleven', 'nm.', 'numero', 'number', 'nb.', 'rechts', 'dhl', 'links',
               'hinterhaus', 'hausnummer', '1-hg', '2-hg', '3-hg', '4-hg', '5-hg', '6-hg', '7-hg', '1-hoog', '2-hoog',
               '3-hoog', '4-hoog', '5-hoog', '6-hoog', '7-hoog', 't/m', 'T/m', 'T/M', 't/M', 'hoog', 'Hoog', 'HOOG',
               'huis', 'laag', 'beneden', 'boven']
remove_list_with_number = ['vloer', 'floor', 'hal', 'vloer', 't.a.v.', 'afd.', 'verd.', 'etage', 'étage', 'bus', 'room',
                           'vrt', 'bte', 'tor', 'boite']
verdiepingen = ['1e', '2e', '3e', '4e', '5e', '6e', '7e', '8e', '9e', '1th', '2th', '3th', '4th', '5th', '6th', '7th',
                '8th', '9th']
blacklist_street = []
afkorting = {}
blacklist_general = []
straat_eind_cijfer = []
straateind = ['str', 'weg', 'väg', 'laan', 'ple', 'sing', 'kad', 'plan', 'bazar', 'Tänav', 'super', 'bul', '街', 'Tar',
              'shar', 'hof', 'sor', 'gar', 'lane', 'baan', 'алея', 'alieja', 'ถนน', 'strada', 'utca', 'calle',
              'бульвар', 'Булевард', 'vard', 'aleja', 'οδός', 'unter', 'kamp', 'park', 'haven', 'beach', 'bur', 'and',
              'via', 'maan', 'man', 'aut', 'calle', 'gat']
...
def general_cleaning(input):
    ...
    for index, s in enumerate(split):
        if hasNumbers(s) and hasLetters(s) and '/' not in s and '-' not in s:
            cd = 0
            cl = 0
            for l in s:
                if l.isdigit():
    ...
    input = forgotten_space(clean(remove_double_spaces(input)))
...
def cleaner_postbus_checker(input):
    postbusnum = ''
    ...
    return {'cleaned_up': drop_triplicate_letters(general_cleaning(input)).strip(), 'postbus': postbusnum}
...
def clean_street(item):
    ...
    # except:
    output = item
    return afkorting_to_full(output.strip())
...
def parser(address):
    output = {'streetname': '', 'housenumber': '', 'double_hn': '', 'housenumber_ext': '', 'postcode': '',
              'postbus': '', 'extra': ''}
    try:
        ...
        postbus_check = cleaner_postbus_checker(address)

        address = postbus_check['cleaned_up']
        postbus = postbus_check['postbus']
        ...
        if hasNumbers(address):
            ...
            for i, element in enumerate(split):
                ...
                try:
                    if element.lower() in verdiepingen and split[i + 1].lower() in ['et', 'etage', 'etag', 'et.',
                                                                                    'etag.', 'verdieping']:
                        del split[i]
                        del split[i]
                except:
            ...
            if hasNumbers(split[-1]) == False and split[0].isdigit():
                ...
                split = address.split()
            elif hasNumbers(split[0]) and split[0][0].isdigit() and split[-1][0].isdigit() == False and split[
                0] not in ['1e', '2e', '3e', '4e']:
                address = make_string(split[1:] + [split[0]])
                split = address.split()
            ...
            if le.isdigit():
                if len(le) < 8 and len(le) > 4 and le.isdigit() and int(le[0:4]) in list(range(1940, 1946)):
                    ...
                    return {'streetname': clean(make_name(street)), 'housenumber': huisnum, 'double_hn': double_hn,
                            'housenumber_ext': '', 'postcode': postcode.upper(), 'postbus': postbus}
                ...
                elif '-' in le2 and le2.split('-')[-1].isdigit():
                    if split[-3].isdigit() == False and make_string(split[0:-2]).replace(' ', '').replace(
                            le2.split('-')[-1], '') in straat_eind_cijfer:
                        blacklist_used = True
                        huisnum = le
                        street = make_string(split[0:-1])
                        huisnum_ext = ''
                    elif split[-3].isdigit() and make_string(split[0:-3]).replace(' ', '').replace(le2.split('-')[-1],
                                                                                                   '') in straat_eind_cijfer:
                        blacklist_used = True
                        huisnum = le2
                        street = make_string(split[0:-2])
                        huisnum_ext = le
                    elif split[-2].isdigit() and make_string(split[0:-2]).replace(' ', '').replace(le2.split('-')[-1],
                                                                                                   '') in straat_eind_cijfer:
                        blacklist_used = True
                        huisnum = le
                        street = make_string(split[0:-2])
            ...
            # if there are no numbers in the last element this is probably an extension because we already removed postcodes.
                ...
                if len(split) == 1:
                    return {'streetname': make_name(afkorting_to_full(split[0])), 'housenumber': '',
                            'double_hn': double_hn, 'housenumber_ext': '', 'postcode': postcode.upper(),
                            'postbus': postbus}
            ...
            # there is probably a split between housenumber and extension given with this dash.
                    ...
                    if len(le_dash_spl) == 1:
                        ...
                        if le_dash_spl[-1].isdigit():
                            ...
                            if le_dash_spl[-2].isdigit():
                                ...
                                return {'streetname': clean_street(make_name(address)), 'housenumber': '',
                                        'double_hn': double_hn, 'housenumber_ext': '', 'postcode': postcode.upper(),
                                        'postbus': postbus}
                            ...
                            return {'streetname': clean_street(make_name(address)), 'housenumber': '',
                                    'double_hn': double_hn, 'housenumber_ext': '',
                                    'postcode': postcode.upper(), 'postbus': postbus}
                ...
                # if still not found, we might have an issue and give back everything as street
                    ...
                    return {'streetname': clean_street(make_name(address)), 'housenumber': '', 'double_hn': double_hn,
                            'housenumber_ext': '', 'postcode': postcode.upper(), 'postbus': postbus}
        ...
        # when there are no numbers in the address whatsoever, we give back the whole address as streetname..
            ...
            # huisnum_ext = ''
            street = address
            return {'streetname': clean_street(make_name(street)), 'housenumber': '', 'double_hn': double_hn,
                    'housenumber_ext': '', 'postcode': postcode.upper(), 'postbus': postbus}
        ...
        if hn_2 != '' and len(hn_2) < 4:
            huisnum = hn_2
            huisnum_ext = hne_2

        if street == '':
            return {'streetname': '', 'housenumber': remove_shit(str(int(huisnum))), 'double_hn': double_hn,
                    'housenumber_ext': remove_shit(huisnum_ext), 'postcode': postcode.upper(), 'postbus': postbus}
        else:
            if blacklist_used:
                return {'streetname': clean(make_name(street)), 'housenumber': remove_shit(str(int(huisnum))),
                        'double_hn': double_hn, 'housenumber_ext': clean_ext(remove_shit(huisnum_ext)),
                        'postcode': postcode.upper()}
            else:
                return {'streetname': clean_street(make_name(street)), 'housenumber': remove_shit(str(int(huisnum))),
                        'double_hn': double_hn,
        ...
        try:
            ...
            for i, element in enumerate(split):
                if element.isdigit():
                    ...
                    huisnum_ext = split[-1]
                    return {'streetname': clean_street(make_name(street)),
                            'housenumber': remove_shit(str(int(huisnum))), 'double_hn': double_hn,
                            'housenumber_ext': clean_ext(huisnum_ext), 'postcode': '', 'postbus': postbus}
        except:
            try:
                return {'streetname': clean_street(make_name(street)), 'housenumber': '', 'double_hn': double_hn,
                        'housenumber_ext': '', 'postcode': postcode.upper(), 'postbus': postbus}
...
def parser_final(input):
    ...
    if output is None:
        ...
        try:
            ...
            output = {'streetname': street, 'housenumber': hn_2, 'double_hn': False,
                      'housenumber_ext': clean_ext(hne_2), 'postcode': '', 'extra': '', 'postbus': ''}
        except:
            try:
                street = clean(clean_street(make_name(input)))
                hn_2, hne_2 = finding_another_hn(input)
